...
const Subscription = () => {
  ...
  useEffect(() => {
    ...
    const fetchCurrentSubscription = async () => {
      ...
      try {
        ...
        // Don't set error if no subscription (404), as it's valid for new users
  ...
  const handleCheckout = async (planId) => {
    ...
    try {
      ...
      if (planId === 'free') {
        ...
        localStorage.setItem('isAdmin', 'true'); // Update isAdmin status
        setCurrentPlan('free'); // Update UI
        setTimeout(() => {
          navigate('/profile');
          setSuccess(null);
        }, 2000);
  ...
  // Admin view for /admin/subscription/switch
  if (isAdmin && location.pathname === '/admin/subscription/switch') {
    return (
      <PayPalScriptProvider options={{ 'client-id': paypalClientId }}>
        <div className="min-h-screen bg-gradient-to-b from-secondary to-white py-12 px-4">
          <div className="max-w-5xl mx-auto">
            <h1 className="text-4xl font-extrabold text-center text-primary mb-2">Switch Your Subscription Plan</h1>
            <p className="text-text text-lg text-center mb-12">Choose a new plan to manage your job posts and applicants.</p>
            {error && (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {plans.map((plan) => (
                <div
                  ...
                  } ${plan.isPopular && currentPlan !== plan.planId ? 'border-2 border-accent' : ''}`}
                >
                  {plan.isPopular && currentPlan !== plan.planId && (
                  ...
                  {currentPlan === plan.planId && (
                    <span className="absolute top-0 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-success text-white text-xs font-bold px-3 py-1 rounded-full">
                      Current Plan
                    </span>
                  )}
  ...
  // Non-admin view for /subscription
  if (!paypalClientId && !isAdmin) {
    return (
              ...
              <div
                ...
                className={`relative bg-white rounded-xl shadow-lg p-6 transform transition-all duration-300 hover:shadow-xl ${
                  selectedPlan === plan.planId && qrCode && paymentId
                    ...
                    : currentPlan === plan.planId
                    ? 'min-h-[350px] border-2 border-success bg-success/5'
